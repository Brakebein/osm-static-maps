...
async function osmsm(options) {
  ...
  try {
    page.on('error', (err) => { throw new Error(err.toString()); });
    page.on('pageerror', (err) => { throw new Error(err.toString()); });
    page.on('console', (msg) => {
    ...
    if (options.imagemin) {
      ...
      return imagemin.buffer(imageBinary, { plugins });
    } else {
      return imageBinary;
    }
  } finally {
